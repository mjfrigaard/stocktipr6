...
**Before**: Limited testing infrastructure (no test files in `stocktipr`)
...
1.  **`mod_inputs.R`** (ModInputs)
    ...
    -   **Public Methods**:
        ...
        -   `ui(...)` - Returns sidebar UI with all input controls; `...` appends extra elements (e.g., `ModDownload$ui()`)
        -   `server()` - Returns reactive list with tickers, dates, vol_window, fetch
...
3.  **`mod_download.R`** (ModDownload)
    ...
    -   **Public Methods**:
        ...
        -   `ui()` - Returns the report format selector and download button card
...
9.  **`utils_logging.R`**
    ...
    -   **Functions**:
        ...
        -   `app_set_log_threshold(level)` - Sets logging threshold across all stocktipr6 namespaces
